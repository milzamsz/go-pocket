...
func RegisterRoutes(se *core.ServeEvent, deps *Deps) {
	// Baseline security headers on every application response.
	se.Router.BindFunc(middleware.SecurityHeaders())

	// Per-IP rate limiting for sensitive auth endpoints.
	authLimiter := middleware.NewRateLimiter(authRateLimit, authRateLimitWindow).Middleware()
	...
	authGroup.POST("/login", handlers.AuthLogin(deps.Auth)).BindFunc(authLimiter)
	authGroup.GET("/signup", handlers.Page("auth-signup"))
	authGroup.POST("/signup", handlers.AuthSignup(deps.Auth)).BindFunc(authLimiter)
	authGroup.GET("/logout", handlers.AuthLogout(deps.Auth))
	authGroup.GET("/forgot-password", handlers.Page("auth-forgot-password"))
	authGroup.POST("/forgot-password", handlers.AuthForgotPassword(deps.Auth)).BindFunc(authLimiter)
	authGroup.GET("/reset-password", handlers.Page("auth-reset-password"))
	authGroup.POST("/reset-password", handlers.AuthResetPassword(deps.Auth)).BindFunc(authLimiter)
	...
	appGroup.GET("/{$}", handlers.AppDashboard())
	appGroup.GET("/onboarding", handlers.AppOnboarding())
	appGroup.POST("/onboarding", handlers.Accepted("app-onboarding"))
	appGroup.POST("/switch-org", handlers.Accepted("app-switch-org"))
	appGroup.GET("/settings/profile", handlers.AppSettingsProfilePage())
	appGroup.POST("/settings/profile", handlers.AppSettingsProfile(deps.Auth))
	appGroup.GET("/settings/security", handlers.AppSettingsSecurityPage())
	...
	appGroup.GET("/settings/account", handlers.AppSettingsAccountPage())
	...
	orgGroup.GET("/{$}", handlers.OrgOverview(deps.Tenancy))
	...
	orgGroup.GET("/products", handlers.ListProducts(deps.Products, deps.Tenancy))
	orgGroup.GET("/products/{id}", handlers.ShowProductDetail(deps.Products, deps.Tenancy))
	...
	// Kanban Board routes
	orgGroup.GET("/kanban", handlers.ShowKanbanBoard(deps.Kanban, deps.Tenancy))
	...
	orgGroup.GET("/invitations", handlers.OrgInvitations(deps.Tenancy))
	...
	orgGroup.GET("/billing", handlers.OrgBilling(deps.Tenancy))
	...
	orgGroup.GET("/settings", handlers.OrgSettingsPage(deps.Tenancy))
	orgGroup.POST("/settings", handlers.OrgSettingsUpdate(deps.Tenancy, deps.Email)).BindFunc(
		middleware.RequireOrgRole(tenancy.PermissionMembersWrite, domain.OrgRoleAdmin),
	)
	orgGroup.GET("/settings/danger", handlers.OrgSettingsDangerPage(deps.Tenancy))
	...
	orgGroup.GET("/audit", handlers.OrgAudit(deps.Tenancy))
	...
	adminGroup.GET("/settings", handlers.AdminSettings())
